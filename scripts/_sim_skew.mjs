...
// 目的: 确认木木点排盘时的错误页形态, 并验证「自动恢复」能否自愈。
//
// 用法:
//   node scripts/_sim_skew.mjs chunk                 持续阻断 → 复现错误页
//   node scripts/_sim_skew.mjs chunk --heal          观测到 ChunkLoadError 即放行 → 验证自愈
//   node scripts/_sim_skew.mjs chunk --brief=3       固定阻断 3 秒后放行
//
// ⚠️ 别改用 page.setRequestInterception 做阻断: 页面加载后再开, 导航会直接卡住
//    （表现成「点了没反应」），白折腾。CDP 的 Network.setBlockedURLs 才是对的。
...
await page.evaluateOnNewDocument(() => {
  // 加载次数: 用来判断「自动恢复」有没有变成无限刷新 (正常 = 1; 恢复一次 = 2)
  try {
...
// ---- 阻断: 页面已就绪, 从这里开始导航所需资源拿不到 (等价于「旧客户端遇上新部署」) ----
...
console.log(
  `阻断模式: ${patterns.join(' , ')}` +
    (HEAL ? ' · 观测到 ChunkLoadError 即放行（自愈验证）' : BRIEF_MS ? ` · ${BRIEF_MS / 1000}s 后解除` : ' · 持续阻断'),
)

// 自愈验证的关键: 故障窗口是"瞬时"的 —— 一旦客户端报出 ChunkLoadError, 就当作故障已过去
if (HEAL) {
  page.on('console', (m) => {
    if (m.text().includes('ChunkLoadError')) {
      console.log('   ⚡ 观测到 ChunkLoadError → 立即放行（模拟故障窗口已过）')
      release()
    }
  })
}
...
if (BRIEF_MS && !HEAL) setTimeout(release, BRIEF_MS)
...
for (let k = 0; k < 60; k++) {
  ...
  } catch { continue } // 导航中 evaluate 会抛, 跳过这一轮
  if (st.errPage) sawErrPage = true
  if (st.healing) sawHealing = true
  // 瞬时模式: 必须等到盘面画出来 (自愈成功) 或超时; 持续阻断: 见到任一终态就停
  if (HEAL || BRIEF_MS ? st.planetGs > 0 : st.errPage || st.boundary || st.planetGs > 0) break
}
...
const persisted = await page
  ...
  .catch(() => ({ healing: false, errPage: false, loads: -1 }))
if (persisted.healing) sawHealing = true
if (persisted.errPage) sawErrPage = true

console.log(`\n落在: ${st?.path}`)
console.log(`过程: 出现过错误页=${sawErrPage} · 出现过恢复态=${sawHealing} · 页面加载次数=${persisted.loads}（1=未刷新, 2=恢复一次, >2=在循环）`)
console.log(`终态: 错误页=${st?.errPage} 盘面卡=${st?.boundary} 行星组=${st?.planetGs}`)
if (st?.bodyHead) console.log(`页面开头: ${st.bodyHead}`)
