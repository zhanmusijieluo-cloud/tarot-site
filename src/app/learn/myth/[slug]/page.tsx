...
/** slug=myth-XX → 大阿卡纳 id */
const cardIdOf = (slug: string) => {
  const m = slug.match(/^myth-(\d+)$/);
  return m ? parseInt(m[1], 10) : null;
};

export default function MythArticlePage() {
  ...
  if (loading) {
    return (
      <PageShell label={t('learn.myth.label')} title={t('learn.myth.title')}>
    ...
    return (
      <PageShell label={t('learn.myth.label')} title={t('learn.myth.title')}>
        <Reveal className="flex flex-col items-center gap-6 py-24">
          ...
          <button onClick={() => router.push('/learn/myth')} className="glass-btn-primary text-sm">
            ← {t('learn.myth.back')}
          </button>
        </Reveal>
      </PageShell>
  ...
  const cardId = cardIdOf(article.slug);
  const card = cardId !== null ? TAROT_DECK.find((c) => c.id === cardId) : null;
  const hasContent = !!(intro || sections.length);

  // 上一篇 / 下一篇（按 myth-XX 编号相邻，仅跳转本篇所在链）
  const prevSlug = cardId !== null && cardId > 0 ? `myth-${String(cardId - 1).padStart(2, '0')}` : null;
  const nextSlug = cardId !== null && cardId < 21 ? `myth-${String(cardId + 1).padStart(2, '0')}` : null;
  ...
  return (
    ...
    >
      <Reveal className="mt-10">
        <button onClick={() => router.push('/learn/myth')} className="text-xs tracking-[0.15em] text-accent/80 transition-colors hover:text-accent">
          ← {t('learn.myth.back')}
        </button>
      </Reveal>
      ...
      {card && (
        <Reveal delay={60}>
          <div className="mx-auto mt-8 flex max-w-3xl items-center gap-5 rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
            <div
              className="w-20 shrink-0 overflow-hidden rounded-lg shadow-lg shadow-black/40 ring-1 ring-white/10"
              style={{ aspectRatio: '2 / 3.4' }}
            >
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={getCardImage(card.id)} alt={card.name} className="h-full w-full object-cover" />
            </div>
            <div className="min-w-0">
              <p className="font-display text-sm tracking-[0.15em] text-frost">{localizedCardName(card, lang)}</p>
              <p className="mt-1 text-[11px] tracking-[0.15em] text-muted/70">{card.numeral} · {t(`card.${card.id}`)}</p>
            </div>
          </div>
        </Reveal>
      )}
