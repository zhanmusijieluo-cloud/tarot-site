...
4.5 **动机视角（贯穿全部板块，但不污染占断）**：回答前先自问——问卜者为什么会来问这个问题？ta 此刻的处境、恐惧、期盼是什么？动机理解只作为两件事使用：① 决定措辞的温度与共情落点；② 让结论的"落点"贴近 ta 的切身关切。**但牌面义理永远是判官——绝不为迎合动机而圆滑牌义、美化或恶化结论**。特别地：若问题可能是「确认型」（客户心里已有答案，仅来确认），解读应诚实呈现牌义证据，允许结论与客户的盼望相反，并温和点出"牌面印证了你心里已有的那个感觉"之类的可能性；占断与期盼冲突时，呈现张力而不调和，把选择权交还客户。
