...
> ⚠️ **本文是较早快照, 部分口径已被后续提交推翻并就地回写**（马盘定义、时空/马盘推运轴两处, 见下文标注）。
> **与 `交接文档.md` 第 5 节 / `prog-cusp-reverse-engineering.md` 冲突时, 以后两者 + 代码为准。**
...
- **16 盘全部落地** (占位与 TODO_NOTE 已全删; SINGLE_TABS 数组泛化渲染, 加盘 = 引擎字段 + chartMap/TABS 条目). 后 10 个的算法口径:
  - **马盘A/B** = `marksBirth(self, davisonIn)` = `davisonBirth(self, davisonIn)` — **Marks 原文定义: 本命盘与戴维森(时空)盘的中点** (日期/时间/地点全部取中点), 代码 `dynamic.ts:525`。**非网传简版「一方时间 + 对方地点」**（旧口径已废, 曾致对拍全盘不符）。爸爸对照测测/爱星盘核准: 马盘A = A ♂ 时空盘 的中点; 马盘B = B ♂ 时空盘 的中点。
  ...
  - **时空三限/次限、马盘A/B三限次限** = `progChart(base盘, baseBirth, mode)` — 对中点盘直接推运; 轴推进走 `advAxes` → `recastAxes(chart, 推进天数 × 0.9856473)` = **Naibod 平均太阳赤经率推进 RAMC + 原地点纬度重排 Placidus（零校准, 2026-09-15 终版）** → **轴随推运推进** (旧版轴定格已修)。**旧「基础轴 + 各自太阳弧 + 校准表」口径对真盘已废** — `PROG_AXIS_CORR`/`PROG_CUSP_CORR` 校准表只留给组合盘家族 (compS/compT); 宫位号按 base 宫头重挂、相位重算.
...
- **组合推运轴公式 = 已锁定 (两年双验, 已实现)**: **ASC = 组合盘 ASC + 组合太阳弧 + 校准量** (校准: 次限 +2.1° / 三限 −2.0°, 经验值待理论化 → 源码 `PROG_AXIS_CORR` 表); **MC = 组合盘 MC + 组合太阳弧** (不加校准)。组合太阳弧 = 双方推运太阳中点 − 双方本命太阳中点。实测: 组合次限 ASC 巨蟹 11.16° vs 爱星盘 11.15° (差 0.01°), 次年同盘差 0.12°; 组合三限 ASC 巨蟹 23.38° vs 测测 23.35° (差 0.03°)。
  ...
  - **时空/马盘推运轴** (davS/davT/marksAS/AT/BS/BT): **已改走 Naibod 率 RAMC 推进 + 原纬度重排 Placidus, 并完成对拍**（时空三限 ≤0.11° / 马盘B三限 ≤0.44°, 零校准常量）。旧「同口径 + 同校准表」说法已废 — 校准表只服务组合家族。详见 `参考资料/prog-cusp-reverse-engineering.md`。
  ...
  - **宫头随轴同步推进 (已实现, 两年双验)**: 推运盘 12 宫头 = 本命盘宫头 + 太阳弧 + 逐宫修正表 `PROG_CUSP_CORR` ([2.11, -0.85, -1.09, -0.14, 0.89, 2.73, 2.11, -0.50, -0.79, -0.14, 1.26, 3.04], 反解自爱星盘组合次限两年截图), 四轴强制锚定 1/4/7/10 = ASC/IC/DSC/MC。实测: 组合次限 12 宫头全对 (max 残差 0.37°); 组合三限中间宫头无独立对照 (扫描图面在像素噪声 ±2° 内与表版一致) → 两模式共用同表。**反解数据/像素扫描法/敏感度见 `参考资料/prog-cusp-reverse-engineering.md`**。
